...
def generate_nightly_pdf(date_str, output_path):
    ...
    target_datetime_start = pd.to_datetime(TARGET_DATE) + pd.Timedelta(hours=12)
    target_datetime_end = pd.to_datetime(TARGET_DATE) + pd.Timedelta(days=1, hours=12)

    # Filter for specific date
    df = df[df['start_timestamp'] > target_datetime_start]
    df = df[df['start_timestamp'] < target_datetime_end]
    # --- INSERT DOWNTIME EVENTS ---
    def insert_downtime(group, date):
        group = group.sort_values('start_timestamp').reset_index(drop=True)
        all_downtime_rows = []

        day_start = target_datetime_start
        day_end = target_datetime_end
    ...
    # Title
    title = Paragraph(f"<b>CNC Machine Nightly Report</b><br/>{TARGET_DATE}", styles['Title'])
    report.append(title)
    report.append(Spacer(1, 24))

    # --- SUMMARY PAGE ---
    summary_data = [['Machine', 'Total Runs', 'Total Runtime', 'Total Downtime', 'Efficiency Rate (%)']]
